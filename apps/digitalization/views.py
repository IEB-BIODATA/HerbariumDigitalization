...
from .forms import LoadPriorityVoucherForm, LoadColorProfileForm, VoucherImportedForm
...
def qr_generator(request):
    ...
    return render(request, 'digitalization/qr_generator.html', {'herbariums':herbariums, 'generated_pages':generated_pages})
...
def code_generator(request):
    if request.method == 'POST':
        ...
        vouchers=VoucherImported.objects.filter(occurrenceID__qr_generated=False, herbarium=herbarium).order_by('-priority','scientificName__genus__family__name','scientificName__scientificName','catalogNumber')[:num_qrs]
        if vouchers.count() > 0:
            generated_pages_count = GeneratedPage.objects.filter(herbarium=herbarium,terminated=False).count()
            if generated_pages_count == 0:
                generated_page=GeneratedPage(name=str(quantity_pages_input)+' páginas - '+str(vouchers.count())+' códigos - Fecha:'+str(datetime.now(tz=pytz.timezone('America/Santiago')).strftime('%d-%m-%Y %H:%M')),herbarium=herbarium, created_by=request.user, created_at=datetime.now(tz=pytz.timezone('America/Santiago')))
                generated_page.save()
                ...
                data = {'result':'OK', 'pages':count_pages, 'generated_page': {'id':generated_page.id, 'herbarium':generated_page.herbarium.name, 'Total':num_qrs, 'created_by':str(generated_page.created_by), 'created_at':generated_page.created_at.strftime('%d de %B de %Y a las %H:%M')}}
            else:
...
def historical_page_download(request):
    if request.method == 'GET':
        ...
        vouchers=VoucherImported.objects.filter(occurrenceID__page=page).order_by('priority','scientificName__genus__family__name','scientificName__scientificName','catalogNumber')
        for voucher in vouchers:
        ...
        result=render_to_pdf('digitalization/template_qr.html',{'pagesize':'A5','codes_list': codes_list,'col':col, 'page_date':page.created_at, 'page_id':page.id})
        delete_tmp_qr()
...
def historical_priority_voucher_page_download(request):
    if request.method == 'GET':
        historical_page_id = request.GET['historical_page_id']
        page = GeneratedPage.objects.get(pk=historical_page_id)      
        priority_vouchers=VoucherImported.objects.filter(occurrenceID__page=page).order_by('priority','scientificName__genus__family__name','scientificName__scientificName','catalogNumber')
        result=render_to_pdf('digitalization/template_list_priority_voucher.html',{'pagesize':'letter', 'page_date':page.created_at, 'page_id':page.id,'priority_vouchers': priority_vouchers})
        return HttpResponse(result, content_type='application/pdf')

@login_required
def load_priority_vouchers_file(request):
    files = PriorityVouchersFile.objects.filter(herbarium__herbariummember__user__id=request.user.id).order_by('created_at')
...
def mark_vouchers(request):
    generated_pages = GeneratedPage.objects.filter(herbarium__herbariummember__user__id=request.user.id).order_by('created_at')
...
def get_vouchers(request):
    if request.method == 'GET':
        ...
        if state_voucher == -1:
            biodata_codes=VoucherImported.objects.filter(Q(occurrenceID__voucher_state=0) | Q(occurrenceID__voucher_state=1) | Q(occurrenceID__voucher_state=2),occurrenceID__page=page).order_by('scientificName','catalogNumber')
        else:
            biodata_codes=VoucherImported.objects.filter(occurrenceID__page=page,occurrenceID__voucher_state=state_voucher).order_by('scientificName__scientificName','catalogNumber')
        data = serializers.serialize('json', biodata_codes, fields=('catalogNumber', 'recordedBy', 'recordNumber', 'scientificName', 'locality', ))
...
def get_digitalization_state(request):
    ...
    if request.method == 'GET':
        ...
        biodata_codes=BiodataCode.objects.filter(code=code_voucher)[0]
        data={'result':'OK', 'id_page' : biodata_codes.page.id,'date':biodata_codes.page.created_at.strftime('%d-%m-%Y'), 'color_profile': str(biodata_codes.page.color_profile.file.url), 'herbarium': biodata_codes.herbarium.collection_code}
    return HttpResponse(json.dumps(data), content_type="application/json")
...
def csv_error_data(request):
    if request.method == 'POST':
        response = HttpResponse(content_type='text/csv',headers={'Content-Disposition': 'attachment; filename=vouchers_error.csv'},)
        ...
        for i in indexes:
            writer.writerow([data_errors['catalogNumber['+str(i)+']'],data_errors['recordedBy['+str(i)+']'],data_errors['recordNumber['+str(i)+']'],data_errors['scientificName['+str(i)+']'],data_errors['locality['+str(i)+']']])
        return response
    ...
    if request.method == 'POST':
        ...
        try:
            ...
            for idx, value in enumerate(data_errors):
                ...
                index = value_str[value_str.find('[')+len('['):value_str.rfind(']')]
                indexes.append(int(index))
            headers=['catalogNumber', 'recordNumber', 'recordedBy','otherCatalogNumbers','locality', 'verbatimElevation', 'decimalLatitude', 'decimalLongitude', 'georeferencedDate', 'scientificName', 'similarity', 'scientificName_similarity', 'synonymy_similarity']
            for i in indexes:
                data.append([data_errors['catalogNumber['+str(i)+']'],data_errors['recordNumber['+str(i)+']'],data_errors['recordedBy['+str(i)+']'],data_errors['otherCatalogNumbers['+str(i)+']'],data_errors['locality['+str(i)+']'],data_errors['verbatimElevation['+str(i)+']'],data_errors['decimalLatitude['+str(i)+']'],data_errors['decimalLongitude['+str(i)+']'],data_errors['georeferencedDate['+str(i)+']'],data_errors['scientificName['+str(i)+']'],data_errors['similarity['+str(i)+']'],data_errors['scientificName_similarity['+str(i)+']'],data_errors['synonymy_similarity['+str(i)+']']])
        except:
            colums=11
            registers=int(len(data_errors)/colums)
            indexes=[]
            for idx, value in enumerate(data_errors):
                ...
                index = value_str[value_str.find('[')+len('['):value_str.rfind(']')]
                indexes.append(int(index))
            headers=['catalogNumber', 'recordNumber', 'recordedBy','otherCatalogNumbers','locality', 'verbatimElevation', 'decimalLatitude', 'decimalLongitude', 'georeferencedDate', 'scientificName']
            for i in indexes:
                data.append([data_errors['catalogNumber['+str(i)+']'],data_errors['recordNumber['+str(i)+']'],data_errors['recordedBy['+str(i)+']'],data_errors['otherCatalogNumbers['+str(i)+']'],data_errors['locality['+str(i)+']'],data_errors['verbatimElevation['+str(i)+']'],data_errors['decimalLatitude['+str(i)+']'],data_errors['decimalLongitude['+str(i)+']'],data_errors['georeferencedDate['+str(i)+']'],data_errors['scientificName['+str(i)+']']])
...
def pdf_error_data(request):
    if request.method == 'POST':
        ...
        data_list=[]
        for i in indexes:
            data_list.append({'catalogNumber':data_errors['catalogNumber['+str(i)+']'],'recordedBy':data_errors['recordedBy['+str(i)+']'],'recordNumber':data_errors['recordNumber['+str(i)+']'],'scientificName':data_errors['scientificName['+str(i)+']'],'locality':data_errors['locality['+str(i)+']']})
        result=render_to_pdf('digitalization/template_list_priority_voucher.html',{'pagesize':'A4', 'page_date':date.today(),'priority_vouchers': data_list})
        return HttpResponse(result, content_type='application/pdf')
...
def control_vouchers(request):
    if request.method == 'GET' and 'state_voucher' in request.GET:
        ...
        if state_voucher == -1:
            biodata_codes=VoucherImported.objects.filter(herbarium__herbariummember__user__id=request.user.id, occurrenceID__qr_generated=True).order_by('scientificName','catalogNumber')
        else:
            biodata_codes=VoucherImported.objects.filter(herbarium__herbariummember__user__id=request.user.id, occurrenceID__qr_generated=True,occurrenceID__voucher_state=state_voucher).order_by('scientificName','catalogNumber')
        data = serializers.serialize('json', biodata_codes, fields=('occurrenceID','catalogNumber', 'recordedBy', 'recordNumber', 'scientificName', 'locality', ))
        json_data=json.loads(data)
        for index,value in enumerate(json_data):
    ...
    vouchers=VoucherImported.objects.filter(herbarium__herbariummember__user__id=request.user.id, occurrenceID__qr_generated=True,occurrenceID__voucher_state=2).order_by('scientificName','catalogNumber')
...
def get_vouchers_to_validate(request):
    if request.method == 'GET' and 'state_voucher' in request.GET:
        ...
        state_voucher = int(request.GET['state_voucher'])
        if state_voucher == -1:
            biodata_codes=VoucherImported.objects.filter(herbarium__herbariummember__user__id=request.user.id, occurrenceID__qr_generated=True,occurrenceID__page=page).order_by('scientificName','catalogNumber')
        else:
            biodata_codes=VoucherImported.objects.filter(herbarium__herbariummember__user__id=request.user.id, occurrenceID__qr_generated=True,occurrenceID__voucher_state=state_voucher,occurrenceID__page=page).order_by('scientificName','catalogNumber')
        data = serializers.serialize('json', biodata_codes, fields=('occurrenceID','catalogNumber', 'recordedBy', 'recordNumber', 'scientificName', 'locality', ))
        json_data=json.loads(data)
        for index,value in enumerate(json_data):
            value['fields']['id']=biodata_codes[index].id
            ...
            value['fields']['image_voucher_jpg_raw_url_public']=biodata_codes[index].image_voucher_jpg_raw_url_public()
...
def get_voucher_info(request):
    if request.method == 'GET':
        ...
        data={'code_voucher':code_voucher,'herbarium_code':herbarium_code,'herbarium_name':herbarium_name,'scientificNameFull':scientificNameFull,'family':family,'catalogNumber':catalogNumber,'recordNumber':recordNumber,'recordedBy':recordedBy,'locality':locality,'identifiedBy':identifiedBy,'dateIdentified':dateIdentified,'georeferencedDate':georeferencedDate,'organismRemarks':organismRemarks}
    else:
        data={'result':'error'}
    return HttpResponse(json.dumps(data), content_type="application/json")    
...
def vouchers_download(request):
    if request.method == 'GET':
        ...
        headers1=['id','file','code','voucher_state','collection_code','otherCatalogNumbers','catalogNumber','recordedBy',
        'recordNumber','organismRemarks','scientificName','locality','verbatimElevation','decimalLatitude',
        'decimalLongitude','identifiedBy','dateIdentified','decimalLatitude_public','decimalLongitude_public','priority']
        species = VouchersView.objects.values_list('id','file','code','voucher_state','collection_code','otherCatalogNumbers',
            'catalogNumber','recordedBy','recordNumber','organismRemarks','scientificName','locality','verbatimElevation'
            ,'decimalLatitude','decimalLongitude','identifiedBy','dateIdentified','decimalLatitude_public',
...
def generate_etiquete(id):
    ...
    if herbarium_code=='CONC':
        ...
        voucher_image_editable.text((((4000-2150)/2)+2150+delta_x,4530+delta_y), herbarium_name, (0, 0, 0), anchor="ms", font=title_font, stroke_width=2, stroke_fill="black")
        number_font = ImageFont.truetype('static/font/arial.ttf',55)
        voucher_image_editable.text((2250+delta_x,4660+delta_y), herbarium_code+' '+str(catalogNumber), (0, 0, 0), font=number_font, stroke_width=2, stroke_fill="black")
        scientificName_font = ImageFont.truetype('static/font/arial_italic.ttf',48)
        voucher_image_editable.text((((4000-2150)/2)+2150+delta_x,4810+delta_y), scientificNameFull+' ', (0, 0, 0), anchor="ms", font=scientificName_font)
        normal_font = ImageFont.truetype('static/font/arial.ttf',48)
        voucher_image_editable.text((((4000-2150)/2)+2150+delta_x,4870+delta_y), family, (0, 0, 0), anchor="ms", font=normal_font)

        if locality:
            voucher_image_editable.text((2250+delta_x,4980+delta_y), locality, (0, 0, 0), font=normal_font)
        
        
        voucher_image_editable.text((2250+delta_x,5130+delta_y), 'Fecha Col. '+georeferencedDate, (0, 0, 0), font=normal_font)
        voucher_image_editable.text((2900+delta_x,5130+delta_y), 'Leg. '+recordedBy+' '+recordNumber, (0, 0, 0), font=normal_font)

        if dateIdentified:
            voucher_image_editable.text((2250+delta_x,5200+delta_y), 'Fecha Det. '+str(dateIdentified), (0, 0, 0), font=normal_font)

        if identifiedBy:
            voucher_image_editable.text((2900+delta_x,5200+delta_y), 'Det. '+str(identifiedBy), (0, 0, 0), font=normal_font)

        if organismRemarks and organismRemarks!='nan':
            observation = textwrap.fill(str(organismRemarks), width=60, break_long_words=False)
            voucher_image_editable.text((2250+delta_x,5350+delta_y), 'Obs.: '+observation, (0, 0, 0), font=normal_font)
    else:
        shape = [(2150, 4650), (4000, 5690)]
        voucher_image_editable.rectangle(shape, fill ='#d7d6e0', outline ="black", width=4)
        title_font = ImageFont.truetype('static/font/arial.ttf',70)
        voucher_image_editable.text((((4000-2150)/2)+2150,4800), herbarium_name, (0, 0, 0), anchor="ms", font=title_font, stroke_width=2, stroke_fill="black")
        number_font = ImageFont.truetype('static/font/arial.ttf',55)
        voucher_image_editable.text((2250,4900), herbarium_code+' '+str(catalogNumber), (0, 0, 0), font=number_font, stroke_width=2, stroke_fill="black")
        scientificName_font = ImageFont.truetype('static/font/arial_italic.ttf',48)
        voucher_image_editable.text((((4000-2150)/2)+2150,5000), scientificNameFull, (0, 0, 0), anchor="ms", font=scientificName_font)
        normal_font = ImageFont.truetype('static/font/arial.ttf',48)
        voucher_image_editable.text((((4000-2150)/2)+2150,5070), family, (0, 0, 0), anchor="ms", font=normal_font)
        ...
        voucher_image_editable.text((2900,5270), 'Leg. '+recordedBy+' '+recordNumber, (0, 0, 0), font=normal_font)
    ...
    if request.method == 'POST':
        ...
        if form.is_valid():
            ...
            if not numpy.isnan(voucher.decimalLatitude) and not numpy.isnan(voucher.decimalLongitude):
                voucher.point=GEOSGeometry('POINT('+str(voucher.decimalLongitude)+' '+str(voucher.decimalLatitude)+')', srid=4326)
                ...
                voucher.point_public=GEOSGeometry('POINT('+str(decimalLongitude_public)+' '+str(decimalLatitude_public)+')', srid=4326)
                voucher.save()
                if voucher.image:
