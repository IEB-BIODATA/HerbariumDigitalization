...
def generate_dwc_archive(self, option: int):
    ...
    try:
        ...
        try:
            ...
            logger.info(f"Generating EML file for Catalog")
            self.update_state(state="PROGRESS", meta={"step": 0, "total": 1, "logs": logger[0].get_logs()})
            catalog = DarwinCoreArchive(eml.package_id)
            catalog.__meta__.__metadata__ = "eml.xml"
            catalog.__metadata__ = eml.eml_object
            ...
            for i, common_name in enumerate(common_objects):
                ...
                for spp in common_name.species_set.all():
                    common_names_result.append([
                        spp.taxon_id, EMLLanguage.ESP, common_name.name_es
                    ])
            catalog.extensions.append(vernacular_extension)
            catalog.extensions[0].as_pandas(_no_interaction=True)
            catalog.extensions[0].pandas = pd.DataFrame(common_names_result, columns=[fields.name for fields in vernacular_extension.__fields__])
            ...
            catalog.extensions.append(distribution_extension)
            catalog.extensions[1].as_pandas(_no_interaction=True)
            catalog.extensions[1].pandas = pd.DataFrame(distribution_results, columns=[fields.name for fields in distribution_extension.__fields__])
